...
class sfMessageSource_iceMySQL extends sfMessageSource
{
  protected 
    $informations  = array(),
    $application   = null,
  ...
  {
    $stm = $this->pdo->prepare("SELECT cat_id, name, date_modified FROM i18n_catalogue");
    $stm->execute();
  ...
  protected function getCatalogueDetails($catalogue = 'messages')
  {
    ...
    $stm = $this->pdo->prepare("SELECT COUNT(*) FROM i18n_trans_unit WHERE cat_id = ?");
  ...
  protected function updateCatalogueTime($cat_id, $variant)
  {
    ...
    $stm = $this->pdo->prepare("UPDATE i18n_catalogue SET date_modified = ? WHERE cat_id = ?");
  ...
  public function &loadData($variant)
  {
    $stm = $this->pdo->prepare(
      "SELECT t.msg_id, t.source, t.target, t.comments
        FROM i18n_trans_unit t, i18n_catalogue c
  ...
  public function getApplicationName()
  {
    ...
    return sfConfig::get('ice_i18n_global_application', sfProjectConfiguration::getActive()->getApplication());
  }
  ...
  function catalogues()
  {
    ...
    $stm = $this->pdo->prepare('SELECT name FROM i18n_catalogue ORDER BY name');
    $stm->execute();
  ...
  {
    $select_message_stm  = $this->pdo->prepare("SELECT * FROM i18n_trans_unit WHERE source = ? AND cat_id = ? LIMIT 1");
    $insert_message_stm  = $this->pdo->prepare("INSERT INTO i18n_trans_unit (cat_id, source, target, date_modified) VALUES (?, ?, ?, ?)");
    ...
    {
      $update_stm = $this->pdo->prepare(sprintf("UPDATE i18n_catalogue SET date_modified = ? WHERE cat_id IN(%s)", implode(',', $in_prepare)));
  ...
  function delete($message, $catalogue = 'messages')
  {
    ...
    $stm = $this->pdo->prepare("DELETE FROM i18n_trans_unit WHERE cat_id = ? AND source = ?");
  ...
  function update($text, $target, $comments, $catalogue = 'messages')
  {
    ...
    $stm = $this->pdo->prepare("UPDATE i18n_trans_unit SET target = ?, date_modified = ? WHERE cat_id = ? AND source = ?");
  ...
  {
    $stm    = $this->pdo->prepare("INSERT INTO i18n_trans_unit (cat_id, source, target, date_added) VALUES (?, ?, ?, ?)");
